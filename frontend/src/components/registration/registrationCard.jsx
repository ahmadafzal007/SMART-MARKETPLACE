...
import { 
  Eye, EyeOff, X, ArrowLeft, Mail, Lock, AlertCircle, 
  UserPlus, LogIn, Loader2 
} from 'lucide-react';
...
const LoginModal = ({ isOpen, onClose, onLoginSuccess }) => {
    ...
    // Login submission using authApi.login remains unchanged
    const handleLoginSubmit = async (e) => {
      ...
      try {
        ...
        onLoginSuccess({ name: response.name, email: response.email });
      } catch (err) {
    ...
    const handleRegisterEmailSubmit = (e) => {
      e.preventDefault();
      setError('');
      // Proceed to password step without sending code
      setStep('register-password');
    };
    ...
    const handleRegisterPasswordSubmit = async (e) => {
      ...
      setError('');
      try {
        // API call to send verification code (without verificationCode)
    ...
    // Verification: submission calls authApi.register with complete details
    ...
    // Resend verification code if timer is 0; API call uses name, email, and password
